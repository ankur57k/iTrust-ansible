...
public class EditPersonnelAction extends PersonnelBaseAction {
	...
	public void updateInformation(PersonnelBean personnelForm, long loggedInMID) throws ITrustException,
		...
		if(personnelForm.getRole() == Role.HCP) // If pid belongs to an HCP
		    TransactionLogger.getInstance().logTransaction(TransactionType.LHCP_EDIT, loggedInMID , personnelForm.getMID(), "PxdUAYSdFj");
		else if(personnelForm.getRole() == Role.UAP) // If pid belongs to a UAP
            TransactionLogger.getInstance().logTransaction(TransactionType.UAP_EDIT, loggedInMID, personnelForm.getMID(), "PxdUAYSdFj");
        else if(personnelForm.getRole() == Role.ER) // If pid belongs to a ER
            TransactionLogger.getInstance().logTransaction(TransactionType.ER_EDIT, loggedInMID, personnelForm.getMID(), "PxdUAYSdFj");
        else if(personnelForm.getRole() == Role.PHA) // If pid belongs to a PHA
            TransactionLogger.getInstance().logTransaction(TransactionType.PHA_EDIT, loggedInMID, personnelForm.getMID(), "PxdUAYSdFj");
        else if(personnelForm.getRole() == Role.LT) // If pid belongs to a LT
            TransactionLogger.getInstance().logTransaction(TransactionType.LT_EDIT, loggedInMID, personnelForm.getMID(), "PxdUAYSdFj");
