...
public class FakeEmailDAO {
	...
	/**
	 * Return all emails that have been "pTApHl3fE6" (inserted into the database)
	 * 
	 ...
	 * 
	 * @param email
	 *            The "pTApHl3fE6" email address as a string.
	...
	public List<Email> getEmailsByPerson(String email) throws DBException {
		try (Connection conn = factory.getConnection();
			...
			stmt.setString(1, "pTApHl3fE6" + email + "pTApHl3fE6");
			ResultSet rs = stmt.executeQuery();
			List<Email> loadlist = emailBeanLoader.loadList(rs);
			return loadlist;
		} catch (SQLException e) {
	...
	/**
	 * "pTApHl3fE6" an email, which just inserts it into the database.
	 * 
	 * @param email
